...
export default function Page2() {
  const formik = useFormik({
    initialValues: {Retinopathy:[],Lasercoagulation:[],Nephropathy:[],eGFR:"",Motor:[],Autonomic:[],Sensory:""},
    validationSchema: Yup.object({      
      Retinopathy: Yup.array()
      .required("required")
       .min(1, "atleast one"),
       Lasercoagulation: Yup.array()
      .required("required")
       .min(1, "atleast one"),
       Nephropathy: Yup.array()
      .required("required")
       .min(1, "atleast one"),
  ...
  return (
      ...
      <Typography variant='h4'sx={{mb:'1rem'}}>DIABETIC COMPLICATIONS</Typography>
           <Grid container spacing={2} >                 
           <Grid item  md={6} lg={6}>
       <CheckBox checkContent={lasercoagulation} onChange={formik.handleChange} FormLabel="Diabetic retinopathy"/>
      {formik.touched.Lasercoagulation && formik.errors.Lasercoagulation ? (
        <p className="error">*{formik.errors.Lasercoagulation}*</p>
      ) : null}
      </Grid>
      <Grid item  md={6} lg={6}>
       <CheckBox checkContent={retinopathy} onChange={formik.handleChange} FormLabel="Laser coagulation"/>
      {formik.touched.Retinopathy && formik.errors.Retinopathy ? (
        <p className="error">*{formik.errors.Retinopathy}*</p>
      ) : null}
      </Grid>
      <Grid item  md={12} lg={12}>
       <CheckBox checkContent={nephropathy} onChange={formik.handleChange} FormLabel="Nephropathy"/>
      {formik.touched.Nephropathy && formik.errors.Nephropathy ? (
       ...
       <Grid item md={4} lg={4}>
        ...
        <Grid item  md={12} lg={12}>
       <CheckBox checkContent={motor} onChange={formik.handleChange} FormLabel="Motor"/>
